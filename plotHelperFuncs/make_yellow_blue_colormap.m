function yellowblue = make_yellow_blue_colormap(minmax_ratio)
%
...
yellow = gray(256);
yellow = [ones(length(yellow),1) yellow(:,1).^0.25 yellow(:,1)];

% values from blue to white
blue = gray(round(256*(minmax_ratio)));
blue = [blue(:,1) blue(:,1) ones(length(blue),1)];

% combine
yellowblue = [blue; [1 1 1]; flipud(yellow)];