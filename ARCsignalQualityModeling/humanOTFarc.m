function [OTF2D, fSupport, wave] = humanOTFarc(pRadius, D0, fSupport, wave, LCAshift)
% Calculate the Marimont-Wandell human OTF, including chromatic aberration
%
% Syntax:
%   [OTF2D, fSupport, wave] = ...
%        humanOTFarc([pRadius = 0.0015], [D0 = 58.8235], [fSupport], [wave])
%
% Description:
...
%    humanOTFarc.m' into the Command Window.
%
% Inputs:
...
% Examples:
%{
    [OTF2D, fSupport, wave] = humanOTFarc(0.0015, 60);
    vcNewGraphWin;
    subplot(1, 2, 1)
...
otf = humanCoreARC(wave, sampleSF, p, D0, LCAshift);
% vcNewGraphWin;
