function oi = oiCreateARC(oiType, varargin)
% Create an optical image structure
%
% Syntax:
%   oi = oiCreateARC(oiType, [varargin])
%
% Description:
...
%    oiCreateARC.m' into the Command Window.
%
% Inputs:
...
%                    oiCreateARC('wvf human', pupilMM, zCoefs, wave, umPerDeg)
...
%{
    ...
    oi = oiCreateARC('human');
%}
%{
    oi = oiCreateARC('diffraction');
    oiPlot(oi, 'lsf wavelength');
%}
%{
    % Mean Thibos et al. data. Not too blurry.
    oi = oiCreateARC('wvf human');
    pupilMM = 2;
    oi = oiCreateARC('wvf human', pupilMM)
    oiPlot(oi, 'lsf wavelength');
...
switch oiType
    case {'default', 'human', 'mwhuman'}
        ...
        oi = oiCreateARC('diffraction limited');
        oi = oiSet(oi, 'diffuser method', 'skip');
        oi = oiSet(oi, 'consistency', 1);
        oi = oiSet(oi, 'optics', opticsCreateARC('human',[],[],wave,[],[],LCAshift));
    ...
    case {'wvfhuman', 'shiftinvariant'}
        ...
        % oi = oiCreateARC('wvf human', pupilMM, zCoefs, wave)
