function [weightsS1_x, weightsRBS1_y, rhoFull, rhoNoColor, rhoColor, aic, aicNoColor, weightsRBSci, trialMeans, meanChanges, deltaR, deltaB, deltaS] = ARCnlz_linearModelnobias(sn,bPLOT,nBoot,lumCutoff)
...
if sum(lumCutoff)>0
    if length(lumCutoff)==1
        ...
        indLowLum = (0.4.*(scaleEquateRB.*AFCp.rgb100(:,1).^gammaFactorR + AFCp.rgb100(:,3).^gammaFactorB)>lumCutoff(1) & ...
                    0.4.*(scaleEquateRB.*AFCp.rgb100(:,1).^gammaFactorR + AFCp.rgb100(:,3).^gammaFactorB)<lumCutoff(2)) & ...
                    (0.4.*(scaleEquateRB.*AFCp.rgb200(:,1).^gammaFactorR + AFCp.rgb200(:,3).^gammaFactorB)>lumCutoff(1) & ...
                    0.4.*(scaleEquateRB.*AFCp.rgb200(:,1).^gammaFactorR + AFCp.rgb200(:,3).^gammaFactorB)<lumCutoff(2));
        % indLowLum = ~indLowLum;
        deltaR = deltaR(indLowLum);
        deltaB = deltaB(indLowLum);
        deltaS = deltaS(indLowLum);
