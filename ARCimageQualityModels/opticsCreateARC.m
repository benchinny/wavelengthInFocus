function [optics, wvfP] = opticsCreateARC(opticsType, varargin)
% Create an optics structure
%
% Syntax:
%   [optics, wvf] = opticsCreateARC(opticsType, [varargin])
%
% Description:
...
%{
    ...
    optics1 = opticsCreateARC('diffraction');
	optics2 = opticsCreateARC('human');     % Marimont and Wandell
	optics3 = opticsCreateARC('wvf human'); % Thibos Zernike
%}
...
switch lower(opticsType)
    ...
    case {'default', 'human', 'humanmw'}
        ...
        optics = opticsHumanARC(pupilRadiusMeters,LCAshift,wave);
...
%---------------------------------------
function optics = opticsHumanARC(pupilRadius,LCAshift,wave)
% Using shift-invariant, place estimated human OTF w,with humanOTF
%
% Syntax:
...
if notDefined('LCAshift'), LCAshift = 0; end
...
[OTF2D, fSupport] = humanOTFarc(pupilRadius, dioptricPower, fSupport, wave, LCAshift);
